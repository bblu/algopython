...
header = {
            "User-Agent": "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2743.116 Safari/537.36"
...
def mmRankitem(url):
    ...
    for i in range(len(pages)):
       print  "Item: http://www.zngirls.com/" + pages[i]+"album/"
...
def getPagePicturess(albumsurl):
    ...
    title = htmlpath.xpath('//div[@class="albumTitle"]/h1/text()')[0]
    dirp = u'"/Users/bblu/Downloads/zngirls/%s"' % title
    print 'title:', dirp
    #os.mkdirs(dirp)
...
def savePictures(itemPagesurl):
    ...
    for i in range(len(pages) ):
        ...
        try:
            ...
            binfile = open('%s/%s.jpg' % (dirp, names[i] ) , "wb")
